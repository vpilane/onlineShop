...
const Driver_home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8080/getAllOrders") // Change to your backend endpoint
      .then(res => res.json())
      .then(data => {
  ...
  const handlePick = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8080/pickOrder", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        const updatedOrders = await fetch("http://localhost:8080/getAllOrders").then(res => res.json());
                  ...
                  <td>
                    {orders.isPicked && !orders.isDelivered && (
                      <button
                        ...
                        onClick={async () => {
                          try {
                            const response = await fetch("http://localhost:8080/deliverOrder", {
                              method: "POST",
                              headers: {
                                "Content-Type": "application/json",
                              },
